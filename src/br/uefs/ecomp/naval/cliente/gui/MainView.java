...
public class MainView {
	...
	private void init(){
		UsuariosOnline users = new UsuariosOnline(this.player, tbPlayers, socketListPlayers);
    	OuveDesafio challenge = new OuveDesafio(socketSend);
    	users.start();
    	challenge.start();
	}
	...
	private class CallToPlay extends MouseAdapter{
		...
		try{
			if  (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2) {
				...
				System.out.println("Player ==> "+player+" was selected");
